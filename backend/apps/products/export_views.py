...
def export_movements_excel(request):
    ...
    movements = StockMovement.objects.select_related(
        'product', 'user').all().order_by('-created_at')
    ...
    for row, movement in enumerate(movements, 2):
        ...
        ws.cell(row=row, column=5, value=tipo_map.get(
        ...
        ws.cell(row=row, column=10,
                value=movement.created_at.strftime("%d/%m/%Y %H:%M"))
...
def export_inventory_report_excel(request):
    ...
    for row, movement in enumerate(recent_movements, 2):
        ...
        ws_movements.cell(row=row, column=1,
                          value=movement.created_at.strftime("%d/%m/%Y %H:%M"))
        ...
        ws_movements.cell(row=row, column=4, value=tipo_map.get(
            movement.movement_type, movement.movement_type))
